...
* [`src/cljs/sudoku_reframe_cljs/`](src/cljs/sudoku_reframe_cljs/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/sudoku_reframe_cljs/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `sudoku-reframe-cljs` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/sudoku_reframe_cljs/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns sudoku-reframe-cljs.example
  (:require [sudoku-reframe-cljs.config :as config])
